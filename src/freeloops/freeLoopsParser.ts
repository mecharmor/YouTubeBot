...
export function parseMaxPagination( html : string ) : number {
    const existingPages: number[] = parse(html).querySelectorAll( 'td > .paged' ).map( ( elem : HTMLElement ) => {
        const pagedNumber : number = Number( elem.innerText );
        if( Number.isNaN( pagedNumber ) ) {
            process.env.DEBUG && console.warn( `Somehow we parsed a NaN from ${ elem.innerText }` );
            return 0;
        }
        return pagedNumber;
    } );
...
export async function getMaxPageCountForSearchTerm( searchTerm : string ) : Promise<number> {
    return parseMaxPagination(
        await fetchFreeLoopsPageHtml( { term : searchTerm, page : 1 } )
    )
}

export async function getAudioUrls(searchTerm: string, page : number = 1) : Promise<FreeLoopsProps[]> {
    if( !KnownWorkingTerms.includes( searchTerm.toLowerCase() ) ) {
        process.env.DEBUG && console.warn( `[warning] term ${ searchTerm } does not exist in the known terms. this may not work...` );
    }
